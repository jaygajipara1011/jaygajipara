# 👋 Hi, I'm Jay Gajipara

💻 Web Developer | PHP Laravel Developer  
📍 Rajkot, Gujarat  

---

## 🚀 About Me
I am a passionate Web Developer with experience in building dynamic websites and admin panels using modern technologies.  
I enjoy solving real-world problems and continuously improving my skills.

---

## 🛠️ Tech Stack

- 💻 Languages: PHP, JavaScript
- ⚙️ Frameworks: Laravel
- 🎨 Frontend: HTML, CSS, Bootstrap
- 🗄️ Database: MySQL
- 🔧 Tools: GitHub, VS Code, Postman, XAMPP

---

## 📂 Projects

### 🚗 Car Showroom Management System
- Built using PHP, MySQL, JavaScript
- Manage cars, customers & sales
- Admin panel with CRUD operations

👉 (Add GitHub Repo Link Here)

---

## 💼 Experience

### Ocean Infotech (2023 - 2025)
- Worked on Live CRM Project
- Developed modules for RGT Export
- Google Drive Integration (Auto folder creation)
- Built multiple admin panel websites

---

## 🎓 Education

- 🎓 B.Tech Computer Engineering (Atmiya University) - 8 CGPA
- 📘 12th - 62%
- 📗 10th - 73%

---

## 📫 Contact Me

📧 Email: jaygajipara07@gmail.com  
📱 Phone: 8401486257  

---

## 🌟 GitHub Stats

![Jay's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight)

---

## 🔥 Top Skills

✔ Laravel Development  
✔ Admin Panel Design  
✔ CRUD Operations  
✔ API Integration  

---

⭐ *Feel free to connect and collaborate!*
